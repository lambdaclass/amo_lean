...
open AmoLean.EGraph.Verified.Bitwise.OptimizedNTTPipeline (FieldConfig optimizedNTTC genOptimizedBenchC genOptimizedBenchC_ultra genOptimizedBenchRust genOptimizedBenchRust_ultra costReport babybearConfig koalabearConfig mersenne31Config goldilocksConfig)
...
def runOneBenchC (hw : HardwareCost) (fd : FieldData) (prim : PrimChoice)
  ...
  if h : parts.length ≥ 6 then
    let amoStr := parts[2]'(by omega)
    let p3Str := parts[3]'(by omega)
    let melemStr := parts[4]'(by omega)
    let diffStr := parts[5]'(by omega)
    let n := 2^logN
    IO.println ""
    IO.println s!"  Result:"
      ...
      for logN in cfg.sizes do
        ...
        for lang in cfg.langs do
          ...
          if lang == .c then
            ...
            -- Output format: name,strategy,amo_us,p3_us,melem,diff%
            if h : parts.length ≥ 6 then
              let amo := parts[2]'(by omega)
              let p3 := parts[3]'(by omega)
              let melem := parts[4]'(by omega)
              let diff := parts[5]'(by omega)
              let pad := String.ofList (List.replicate (12 - fd.name.length) ' ')
