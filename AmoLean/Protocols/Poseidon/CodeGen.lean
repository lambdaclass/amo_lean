...
def genSboxFunction (config : CodeGenConfig) : String :=
  ...
  let glMulHelper := match ft with
    | .Goldilocks => "/* Goldilocks modular multiplication: a * b mod (2^64 - 2^32 + 1) */
static inline uint64_t gl_mul(uint64_t a, uint64_t b) {
    ...
    /* Solinas reduction: 2^64 = 2^32 - 1 (mod p) */
    __uint128_t t = (__uint128_t)lo + (__uint128_t)hi * 0xFFFFFFFFULL;
    uint64_t r = (uint64_t)t + (uint64_t)(t >> 64) * 0xFFFFFFFFULL;
 ...
 */"
  ...
  let mulCalls := chain.map fun (name, a, b) =>
    ...
    | .Goldilocks =>
      if name == "res" then s!"    return gl_mul({aVar}, {bVar});  // {name} = {a} * {b}"
      else s!"    {name} = gl_mul({aVar}, {bVar});  // {name} = {a} * {b}"
