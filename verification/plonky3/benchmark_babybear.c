/*
 * Plonky3 Real BabyBear NTT Benchmark
 *
 * Measures actual Plonky3 (monty-31 + NEON) NTT performance for BabyBear.
 ...
 * Format: field,log_n,p3_real_us,melem_s
 *
 * Usage:
 *   make benchmark_babybear
 *   ./benchmark_babybear [log_n_min] [log_n_max] [iters]
 *   # Default: log_n 14..20, 10 iterations
 */
...
#define BABYBEAR_P 2013265921U

/* Function pointers loaded from shim */
typedef int (*ntt_fn)(uint32_t* data, size_t len);
typedef uint32_t (*prime_fn)(void);
typedef uint32_t (*omega_fn)(size_t log_n);
...
static void init_data(uint32_t *data, size_t n) {
    /* Same deterministic init as AMO benchmark: data[i] = (i * 1000000007) % p */
...
int main(int argc, char **argv) {
    ...
    ntt_fn p3_ntt = (ntt_fn)dlsym(lib, "plonky3_babybear_ntt_forward");
    prime_fn p3_prime = (prime_fn)dlsym(lib, "plonky3_babybear_prime");

    if (!p3_ntt || !p3_prime) {
        fprintf(stderr, "ERROR: Cannot find BabyBear symbols: %s\n", dlerror());
        dlclose(lib);
        return 1;
    }
    ...
    /* Header */
    printf("# Plonky3 Real BabyBear NTT Benchmark\n");
    ...
    for (int log_n = log_n_min; log_n <= log_n_max; log_n += 2) {
        ...
        for (int it = 0; it < iters; it++) {
            ...
            total_us += time_ms(&t0, &t1) * 1000.0; /* ms → us */
        }

        double avg_us = total_us / iters;
        double melem = (double)n / avg_us; /* Melem/s */

        printf("babybear,%d,%zu,%.1f,%.1f\n", log_n, n, avg_us, melem);
        fflush(stdout);
